...
async function loadLayoutComponents() {
    try {
        // 1. Fetch and inject global header component layout mapping parameters from absolute root
        const headerResponse = await fetch('/header.html');
        document.getElementById('global-header').innerHTML = await headerResponse.text();

        // 2. Fetch and inject global footer component layout mapping parameters from absolute root
        const footerResponse = await fetch('/footer.html');
        document.getElementById('global-footer').innerHTML = await footerResponse.text();

        // 3. Extract the clean pathname for active route tracking
        ...
        document.querySelectorAll('[data-page]').forEach(link => {
            ...
            // Check if the current path matches the link destination exactly, or matches the parent folder structure
            if (
                currentPath === linkHref || 
                (linkHref !== '/' && currentPath.startsWith(linkHref)) ||
                (linkHref === '/' && (currentPath === '' || currentPath === '/index.html'))
            ) {
